...
exports.onCreateNode = async ({
  ...
  if (node.internal.type.match(/KenticoCloudItem/)) {
    await factoriesManager.createNode({
      node,
      actions,
      store,
      cache,
...
exports.createPages = async ({ graphql, actions }) => {
  ...
  const aboutUsNodes = new Array(
    ...result.data.allKenticoCloudItemAboutUs.edges,
  );
  aboutUsFactory.createPage(aboutUsNodes, createPage);

  const itineraryPages = new Array(
    ...result.data.allKenticoCloudItemItinerary.edges,
  );
  itineraryFactory.createPage(itineraryPages, createPage);

  const landingPages = new Array(
    ...result.data.allKenticoCloudItemLandingPage.edges,
  );
